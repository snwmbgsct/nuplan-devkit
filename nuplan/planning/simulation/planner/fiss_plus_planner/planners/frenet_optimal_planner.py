...
from nuplan.planning.simulation.planner.fiss_plus.planners.common.cost.cost_function import CostFunction
from nuplan.planning.simulation.planner.fiss_plus.planners.common.geometry.cubic_spline import CubicSpline2D
...
from nuplan.planning.simulation.planner.fiss_plus.planners.common.vehicle.vehicle import Vehicle
...
class FrenetOptimalPlanner(object):
    ...
    def has_collision(self, traj: FrenetTrajectory, obstacles: list, time_step_now: int = 0, check_res: int = 1) -> tuple:
        ...
        final_time_step = len(obstacles)#obstacles[0].prediction.final_time_step #? what is final_time
        ...
        for i in range(t_step_max):
            if i%check_res == 0:
                ...
                try:
                    ...
                    for obstacle in obstacles[t_step].tracked_objects:
                        state = obstacle.center
                        if state is not None:
                            obstacle_polygon = self.construct_polygon(obstacle._box.geometry, state.x, state.y, state.heading)
                            num_polys += 1
